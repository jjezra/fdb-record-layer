...
public abstract class StandardIndexMaintainer extends IndexMaintainer {
    ...
    public <M extends Message> List<IndexEntry> filteredIndexEntries(@Nullable final FDBIndexableRecord<M> savedRecord) {
        ...
        final IndexMaintenanceFilter.IndexValues filterType = IndexMaintenanceUtils.getFilterTypeForRecord(state, savedRecord);
    ...
    private boolean isWriteOnlyOrUniquePending() {
        return state.store.getIndexState(state.index).isWriteOnly() || state.store.getIndexState(state.index).isReadableUniquePending();
    }
