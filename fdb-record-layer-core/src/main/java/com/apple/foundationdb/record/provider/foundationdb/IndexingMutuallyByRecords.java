 ...
 * For flexibility, we wish to be able to build index(s) by multiple entities. Whilst:
 *   - Be consistent - each process can bring the indexing process to success.
 *   - Be efficient - all the processes should participate in the built until success.
 ...
 *   - Be able to add indexing processes on the flight.
...
public class IndexingMutuallyByRecords extends IndexingBase {
    ...
    private CompletableFuture<Boolean> buildRangeOnly(@Nonnull FDBRecordStore store,
        ...
        return rangeSet.missingRanges(store.ensureContextActive()).asList().thenCompose(missingRanges ->
    ...
    private CompletableFuture<Boolean> buildThisRangeOnly(@Nonnull FDBRecordStore store, @Nonnull AtomicLong recordsScanned,
        ...
        AsyncIterator<Range> ranges = targetRangeSets.get(0).missingRanges(store.ensureContextActive(), thisRange).iterator();
